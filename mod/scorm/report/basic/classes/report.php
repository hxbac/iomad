    ...
    public function display($scorm, $cm, $course, $download) {
        ...
        if ( !$nostudents ) {
            ...
            if ($detailedrep && $scoes = $DB->get_records('scorm_scoes', array("scorm" => $scorm->id), 'sortorder, id')) {
                foreach ($scoes as $sco) {
                    if ($sco->launch != '') {
                        ...
                        $headers[] = format_string('Trạng thái');
                    }
                }
            } else {
                $scoes = null;
            ...
            if (!$download) {
                ...
                foreach ($headers as $item) {
                  	//thanhlv, thanpt - start
                  	if($item == $headers[count($headers)-1])  $item = "Trạng Thái";
                  	//thanhlv, thanpt - end
                    $myxls->write(0, $colnum, $item, $formatbc);
                  	//thanhlv, thanpt - start
                  	array_push($withLength_arr,mb_strlen($item));
                  	//thanhlv, thanpt - end
                    $colnum++;
                }
                $rownum = 1;
                                ...
                                if ($trackdata = scorm_get_tracks($sco->id, $scouser->userid, $scouser->attempt)) {
                                    ...
                                    if (!$download) {
                                        ...
                                        $row[] = $OUTPUT->pix_icon($trackdata->status, $strstatus, 'scorm') . '<br>' .
                                           \html_writer::link('#', $score, array('title' => get_string('details', 'scorm')));
                                    } else {
                                        $row[] = $score;
                                    }
                                } else {
                    ...
                    if (!$download) {
                        ...
                        foreach ($row as $item) {
                            $myxls->write($rownum, $colnum, $item, $format);
                          	//thanhlv, thanpv - start
                          	if($withLength_arr[$colnum] < mb_strlen($item)){
                          		$withLength_arr[$colnum] = mb_strlen($item);    
                            }
                        	//thanhlv, thanpv - end
                            $colnum++;
                        }
